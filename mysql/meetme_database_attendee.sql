...
CREATE TABLE `attendee` (
  ...
  `Attendee_location` varchar(500) NOT NULL,
  `starttime` timestamp NULL DEFAULT NULL,
  `endtime` timestamp NULL DEFAULT NULL,
  `meetingstatus` enum('PENDING','ATTENDING') NOT NULL DEFAULT 'PENDING',
  `scheduleid` int NOT NULL,
  PRIMARY KEY (`meetid`),
  UNIQUE KEY `attendeeid_UNIQUE` (`meetid`),
  KEY `scheduleFK_idx` (`scheduleid`),
  CONSTRAINT `scheduleFK` FOREIGN KEY (`scheduleid`) REFERENCES `convenororganiser` (`scheduleid`) ON DELETE CASCADE ON UPDATE CASCADE
